...
类似于传统银行的账户，Solana账户可以持有称为[lamports](#lamports)的资金。如同Linux系统中的文件，它可以通过一个键（通常称为[公钥](#公钥)或pubkey）来寻址。
...
## 领导者计划
将[验证者](#验证者)[公钥](#公钥)映射到[插槽](#插槽)的序列。集群使用[领导者](#领导者)计划来确定任何时刻的领导者。
