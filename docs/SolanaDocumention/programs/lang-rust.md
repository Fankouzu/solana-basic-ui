...
有关更多信息，请参阅[sysvar示例测试](https://github.com/solana-labs/solana-program-library/blob/master/examples/rust/sysvar/tests/functional.rs)，该示例展示了如何发送和处理包含sysvar账户的指令。
...
程序被限制为确定性运行，因此无法使用随机数。有时程序可能依赖于一个依赖`rand`的crate，即使程序本身不使用任何随机数功能。如果程序依赖于`rand`，编译将失败，因为Solana不支持`get-random`。错误通常如下所示：
...
支持完整的崩溃消息的默认实现的一个副作用是，程序会引入更多Rust的`libstd`实现到程序的共享对象中。典型程序已经引入了大量的`libstd`，可能不会注意到共享对象大小的显著增加。但那些明确尝试通过避免`libstd`来保持非常小的程序可能会受到显著影响（约25kb）。为了消除这种影响，程序可以提供自己的空实现的自定义崩溃处理程序。
...
[Solana程序库GitHub](https://github.com/solana-labs/solana-program-library/tree/master/examples/rust)仓库包含一系列Rust示例。

[Solana开发者程序示例GitHub](https://github.com/solana-developers/program-examples)仓库也包含一系列从初学者到中级的Rust程序示例。
