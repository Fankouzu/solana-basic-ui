...
在加密扩展中使用的零知识证明可以分为两类：“sigma协议”和“bulletproofs”。Sigma协议是为加密扩展定制的简单系统。Bulletproofs是一种现有的、源于研究论文【[Bulletproofs: Short Proofs for Confidential Transactions and More](https://eprint.iacr.org/2017/1066)】的范围证明系统。
...
转账指令关联有三个 ElGamal 公钥：发送方、接收方和审计方。转账指令数据必须包含这三个加密公钥。
...
除了这些密文外，转账数据还必须包含证明这些密文正确生成的证据。用户有可能通过两种方式欺骗系统。首先，用户可能提供格式错误的密文。例如，即便用户可能使用了错误的公钥加密转账金额，系统也没有办法检查密文的有效性。因此，我们要求转账数据必须包含一个*密文有效性*证明，以证实密文是正确生成的。

密文有效性证明仅能保证twisted ElGamal密文是适当生成的，但它并不认证关于密文中加密金额的任何属性。例如，恶意用户可以加密负数，但仅通过检查密文，系统没有办法检测到这一点。因此，除了密文有效性证明外，转账指令还必须包含一个*范围证明*，以证明加密金额`amount_lo`和`amount_hi`分别是正的16位和32位数值。
...
验证费密文部分涉及到`TransferWithFee`指令数据中用于确认加密费用有效性的必要字段。由于费用是加密的，Token程序不能仅通过检查密文来验证费用是否正确计算。因此，`TransferWithFee`必须包含三个额外的证明来确保费用密文的有效性：

1. **密文有效性证明**：这部分证明确保实际的费用密文是在正确的接收方以及提现扣留授权的ElGamal公钥下正确生成的。

2. **费用sigma证明**：结合范围证明组件，费用sigma证明确保在`fee_ciphertext`中加密的费用根据费用参数被适当地计算。

3. **范围证明**：结合费用sigma证明组件，范围证明组件确保在`fee_ciphertext`中的加密费用根据费用参数被适当地计算。
